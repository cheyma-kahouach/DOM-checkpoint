let hearts = Array.from(document.querySelectorAll("#heart"));

for(let i in hearts){
    hearts[i].addEventListener("click",function(){
        if(hearts[i].getAttribute('class') == "bi bi-heart")
        hearts[i].setAttribute('class','bi bi-heart-fill');
        else
        hearts[i].setAttribute('class','bi bi-heart');
    });
}

let quantitys = Array.from(document.querySelectorAll("#quantity"));
let prices = Array.from(document.querySelectorAll("#price"));


let arrOfQuantitys = [];
let arrOfPrices = [];
for(let i in quantitys){
    arrOfQuantitys[i] = 1 ;
}
for(let i in prices ){
    arrOfPrices[i] = +prices[i].innerHTML;
}

function updateTotal(){
    let total = 0 ;
    for(let i in quantitys)
    total += arrOfPrices[i]*arrOfQuantitys[i];

    document.getElementById('total').textContent=`TOTAL : ${total} $`;
}
updateTotal();

let plusButtons = Array.from(document.querySelectorAll("#plus"));
let minusButtons = Array.from(document.querySelectorAll("#minus"));

for(let i in addButtons){
    addButtons[i].addEventListener("click",function(){
        arrOfQuantitys[i]++;
        quantitys[i].innerHTML = arrOfQuantitys[i];
        updateTotal();
    });
}

for(let i in minusButtons){
    minusButtons[i].addEventListener("click",function(){
        if(arrOfQuantitys[i] > 1){
            arrOfQuantitys[i]--;
            quantitys[i].innerHTML = arrOfQuantitys[i];
            updateTotal();
        }
    });
}

let parents = Array.from(document.querySelectorAll("#item"));
let deletes = Array.from(document.querySelectorAll("#delete"));
for(let i in deletes){
    deletes[i].addEventListener("click",function(){
        arrOfQuantitys[i] = 0;
        updateTotal();
        parents[i].remove();
    });
}





